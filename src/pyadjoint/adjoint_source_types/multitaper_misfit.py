...
:copyright:
    Yanhua O. Yuan (yanhuay@princeton.edu), March-2015
    Yanhua O. Yuan (yanhuay@princeton.edu), modified April-2015
:license:
...
def mt_measure(d1, d2, tapers, wvec, df,nfreq_min, nfreq_max,cc_tshift,cc_dlnA):
    nlen_T = len(d1)
    ntaper = len(tapers[0])
    ...
    phi_w[nfreq_min:nfreq_max] = np.arctan2(trans_func[nfreq_min:nfreq_max].imag, trans_func[nfreq_min:nfreq_max].real)
...
def mt_error(d1, d2, tapers, wvec,df, nfreq_min,nfreq_max, cc_tshift, cc_dlnA, phi_mtm, abs_mtm, dtau_mtm, dlnA_mtm):
    nlen_T = len(d1)
    ntaper = len(tapers[0])
    ...
    for itaper in range(0, ntaper):
        ...
        phi_om, abs_om, dtau_om, dlnA_om = \
          mt_measure(d1, d2, tapers_om, wvec, df, nfreq_min,nfreq_max, cc_tshift, cc_dlnA)
        ...
        eabs_ave[nfreq_min:nfreq_max] = eabs_ave[nfreq_min:nfreq_max] + \
             ntaper * abs_mtm[nfreq_min:nfreq_max] -(ntaper-1) * abs_mul[nfreq_min:nfreq_max,itaper]
...
def mt_adj(d1, d2,deltat,tapers, dtau_mtm, dlnA_mtm, df,nfreq_min, nfreq_max,err_dt_cc,err_dlnA_cc,err_dtau_mt,err_dlnA_mt):
    ...
    for itaper in range(0, ntaper):
        ...
        p_w[nfreq_min:nfreq_max] = d2_tvw[nfreq_min:nfreq_max,itaper] / (bottom_p[nfreq_min:nfreq_max])
        q_w[nfreq_min:nfreq_max] = - d2_tw[nfreq_min:nfreq_max,itaper] / (bottom_q[nfreq_min:nfreq_max])
